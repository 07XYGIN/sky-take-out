...
import { Avatar, Breadcrumb, Button, Dropdown, Layout, Menu, Tooltip } from 'antd'
    ...
    <Layout className="min-h-screen bg-canvas">
      ...
      >
        <div className="flex h-20 items-center gap-3 border-b border-white/10 px-5">
          <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-md bg-saffron text-sm font-black text-ink">天</div>
          {!collapsed && <div className="min-w-0 text-white"><p className="truncate text-[15px] font-semibold">苍穹外卖</p><p className="mt-0.5 text-[10px] uppercase tracking-[0.18em] text-white/45">store operations</p></div>}
        </div>
        <div className="px-3 pt-5">
          {!collapsed && <p className="mb-3 px-3 text-[10px] font-semibold uppercase tracking-[0.18em] text-white/35">workspace</p>}
          <Menu
            theme="dark"
            mode="inline"
            items={menuItems}
        ...
        <div className="absolute bottom-5 left-4 right-4 border-t border-white/10 pt-4 text-xs text-white/40">
          {!collapsed && <div className="flex items-center gap-2"><Settings2 size={14} /> <span>门店运营台</span></div>}
        </div>
      </Sider>
      <Layout>
        <Header className="flex items-center justify-between border-b border-slate-200 bg-white px-4 sm:px-7">
          <div className="flex items-center gap-3">
            <Tooltip title={collapsed ? '展开导航' : '收起导航'}>
              <Button type="text" aria-label={collapsed ? '展开导航' : '收起导航'} icon={collapsed ? <ChevronRight size={18} /> : <ChevronLeft size={18} />} onClick={() => setCollapsed(!collapsed)} />
            </Tooltip>
            <Button className="lg:hidden" type="text" aria-label="打开导航" icon={<MenuIcon size={18} />} onClick={() => setMobileOpen(true)} />
            <Breadcrumb items={[{ title: '苍穹外卖' }, { title: routeNames[selectedKey] || '页面' }]} />
          </div>
          <Dropdown menu={{ items: accountItems, onClick: handleAccountAction }} placement="bottomRight" trigger={['click']}>
            <button className="flex items-center gap-2 rounded-md px-2 py-1.5 text-left transition hover:bg-slate-100" type="button">
              <Avatar size={30} src={user?.avatar} icon={<UserRound size={16} />} className="bg-ink" />
              <span className="hidden text-sm font-medium text-ink sm:block">{user?.name || user?.username || '管理员'}</span>
            </button>
          </Dropdown>
        </Header>
