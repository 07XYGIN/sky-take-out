...
export function SetmealPage() {
  ...
  const openModal = (record?: Setmeal) => {
  ...
  const submit = async (values: Partial<Setmeal>) => {
    try {
      if (editing) await editSetmeal({ ...values, id: editing.id });
      else await addSetmeal(values);
  ...
  const columns: TableColumnsType<Setmeal> = [
    {
      ...
      render: (value, record) => (
        <div className="flex items-center gap-3">
          <div className="flex h-10 w-10 items-center justify-center overflow-hidden rounded-md bg-[#e8f5f3] font-semibold text-teal">
            {record.image ? (
              <img
                src={record.image}
                alt=""
        ...
        <div className="flex items-center gap-3">
          <Switch
            ...
            onChange={async (checked) => {
              try {
                await changeSetmealStatus(record.id, checked ? 1 : 0);
                setRows((current) =>
                  current.map((item) =>
                    item.id === record.id
  ...
  return (
    <div>
      <PageHeader
        title="套餐管理"
        description="编排组合商品，维护套餐价格和供应状态。"
        actions={
          <Button
            type="primary"
            icon={<Plus size={16} />}
      ...
      <Card bodyStyle={{ padding: 0 }}>
        <Table
          ...
          locale={{ emptyText: <EmptyState label="暂无套餐记录" /> }}
